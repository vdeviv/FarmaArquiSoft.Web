            ...
            try
            {
                ...
                if (response.StatusCode == HttpStatusCode.BadRequest)
                {
                    ...
                    ApiValidationFacade.MapValidationErrors(
                        ...
                        idKeywords: new[] { "telefono", "tel�fono", "phone", "celular" }
                    );
                ...
                ModelState.AddModelError(string.Empty,
                    $"Error inesperado del API. C�digo: {(int)response.StatusCode}, Detalle: {response.ReasonPhrase}");
                return Page();
            }
            catch (HttpRequestException ex)
            {
                ModelState.AddModelError(string.Empty,
                    $"Error de conexi�n con el API de Proveedores: {ex.Message}. Verifica que el microservicio de Proveedores est� en ejecuci�n y la BaseAddress sea correcta.");
                return Page();
            }
            catch (Exception ex)
            {
